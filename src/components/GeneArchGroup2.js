...
class GeneArchGroup extends Component {
    ...
    render(){
        ...
        const data = leftJoinByAttr(filterDataByParams(this.props.data, this.props.params), this.props.template, ['position'], {positional_map: 'ind'});
        ...
        if(this.props.useLocalParams){
            ...
            const selectors = this.props.paramOptions.map((d, i) => {
                return (
                    <ParamSelector key={i}
                        className={'genome-chart-param-selector'}
